...
async def recall_node(state: AgentState) -> AgentState:
    ...
    # ── Layer 0 ──
    g0 = input_gate(nc)
    trace.append({"layer": 0, "pass": g0.pass_, "reason": g0.reason})
    if not g0.pass_:
        state["verdict"] = "MISS"
        state["recall_results"] = []
        state["trace_detail"] = trace
        record_transition(state, AgentStatus.RECALLED, f"L0 FAIL: {g0.reason}")
        return state
    ...
    # ── Layer 1 ──
    try:
        query_vec = embedding.encode(nc.core_concept)
    except Exception as e:
        # Embedding 模型未就绪，降级为全零向量 → Dense 搜索将返回空
        query_vec = np.zeros(1024, dtype=np.float32)
        trace.append({"layer": 1, "warning": f"Embedding not ready: {e}"})

    g1 = embedding_quality_gate(query_vec)
    trace.append({"layer": 1, "pass": g1.pass_, "reason": g1.reason})
    if not g1.pass_:
        state["status"] = AgentStatus.ERROR
        state["errors"].append({"node": "recall", "layer": 1, "reason": g1.reason})
        state["trace_detail"] = trace
        record_transition(state, AgentStatus.ERROR, f"L1 FAIL: {g1.reason}")
        return state

    # ── Layer 2 ──
    try:
        candidates, search_meta = hybrid_search(query_vec, nc.keywords, milvus)
    except Exception as e:
        candidates, search_meta = [], {"error": str(e)}
    trace.append({"layer": 2, "candidates": len(candidates), "meta": search_meta})

    if not candidates:
        state["verdict"] = "MISS"
        state["recall_results"] = []
        state["trace_detail"] = trace
        record_transition(state, AgentStatus.RECALLED, "L2 FAIL: 混合检索无结果")
        return state

    # ── Layer 3 ──
    candidates = await dual_scoring(nc, candidates, embedding, llm)
    ...
    cv_summary = [{"topic_id": c.topic_id, "cv": c.cross_validation} for c in candidates if c.passed]
    trace.append({"layer": 4, "cross_validation": cv_summary})
    ...
    state["recall_results"] = [c.model_dump() for c in candidates]
    state["best_match"] = best.model_dump() if best else None
    state["trace_detail"] = trace

    record_transition(state, AgentStatus.RECALLED, f"verdict={verdict}, candidates={passed_count}")
    return state
